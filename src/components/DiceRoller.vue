...
<template>
    ...
    <div v-if="playerResults">
        <div>
            <p>You got {{ playerResults?.rolls[0].value }} Success{{ playerResults?.rolls[0].value == 1 ? '' : 'es' }}
            </p>
            <ul>
                <li v-for="(roll, index) in playerResults?.rolls[0].rolls">
                    Result: {{ roll.value }} - {{ roll.calculationValue == 1 ? 'Success' : 'Fail' }}</li>
            </ul>
        </div>
        <div>Friend Computer {{ computerResult.rolls[0].value == 1 ? `Noticed you` : `Didn't notice you` }}</div>
    </div>
</template>